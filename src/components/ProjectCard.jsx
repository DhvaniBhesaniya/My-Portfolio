...
export default function ProjectCard({ project, compact = false }) {
  ...
  return (
    ...
    >
      <div className="glass-panel glass-panel-hover rounded-2xl overflow-hidden h-full flex flex-col relative">
        <BorderBeam 
          duration={6} 
          size={200} 
        ...
        <div className={`relative overflow-hidden bg-white/5 ${compact ? "h-40" : "h-56"}`}>
        ...
        <div className={`flex flex-col flex-1 gap-3 relative z-10 bg-gradient-to-t from-[#09090B] via-[#09090B]/95 to-transparent backdrop-blur-sm ${
          compact ? "p-4 -mt-12 pt-14" : "p-6 -mt-16 pt-20"
        }`}>
          <h3 className={`text-white/90 font-semibold leading-tight group-hover:text-teal-400 transition-colors duration-300 ${
            compact ? "text-base" : "text-xl"
          }`}>
            {title}
          </h3>
          <p className={`text-white/50 leading-relaxed flex-1 font-light line-clamp-2 ${
            compact ? "text-xs" : "text-sm"
          }`}>
            {description}
          </p>

          <div className={`flex flex-wrap gap-1.5 py-2 border-y border-white/5`}>
            {tags.slice(0, compact ? 3 : 4).map((tag) => (
              <span 
                key={tag} 
                className={`px-2 py-0.5 rounded-full bg-white/5 text-white/60 border border-white/5 group-hover:border-white/10 group-hover:bg-white/10 transition-all duration-300 ${
            ...
            {repoUrl && (
              ...
              >
                <a href={repoUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2">
                  <FaGithub size={compact ? 13 : 15} className="group-hover/btn:scale-110 transition-transform text-white/70 group-hover/btn:text-white" />
                  Source
                </a>
              </Button>
            )}
